...
# ╔═╡ 4be1007a-c50a-47bb-85c9-2dec9558381a
using Plots, PlutoUI
...
_Tu respuesta va aquí._ 
	$$x_{i+1} = x_{0} - \frac{f(x_{0})}{f′(x_{0})} - \frac{f(x_{1})}{f′(x_{1})} - \frac{f(x_{2})}{f′(x_{2})} + \dots + \frac{f(x_{i})}{f′(x_{i})}$$
"""
...
function newton(f, f′, x_0, n)
	raiz = x_0 - (f(x_0)/f′(x_0))
	for i in 1:1:n
		raiz = raiz - (f(raiz) / f′(raiz))
...
# ╔═╡ 536e7dc0-e9b1-44fd-b964-eb24f89bcace
begin
	g(x) = x^2
	g′(x) = 2*x
end

# ╔═╡ 78ecc0cd-e5b2-41b1-a158-5b0096a8d15e
newton(g, g′, 0.1, 2)
...
# Tu código (comentado) va aquí :D
function newtonRecursiva(f, f′, x_0, n)
	raiz = x_0 - (f(x_0)/f′(x_0))
	n -= 1
	if n!=0
		newtonRecursiva(f, f′, raiz, n)
	end
print(raiz)
end

# ╔═╡ 91c8eaaa-29f8-437b-8b6b-fcf650c59392
newtonRecursiva(g, g′, 0.1, 1)
...
# Tu código (comentado) va aquí :D
derivadaNumerica(f,x,h) = h * (f(x+(1/h))-f(x))

# ╔═╡ 0bb6aa70-9931-4739-98c5-644858f4634f
p(x) = x^2

# ╔═╡ 3521f585-ed9e-4acf-825b-ea75e1851b23
x = 2

# ╔═╡ e4a49226-a959-4f9b-a2e9-af8ec1f4f8b5
h = 100000

# ╔═╡ dfaa0974-b402-4ba7-b8db-40971c4a7d09
derivadaNumerica(p,x,h)
...
# Tu código (comentado) va aquí :D
function newtonDN(f,x_0, n)
	dn = derivadaNumerica(g,x_0,10000)
	raiz = x_0 - (f(x_0)/dn)
	for i in 1:1:n
		dn2 = derivadaNumerica(g,raiz,10000)
		raiz = raiz - (f(raiz) / dn2)
...
# ╔═╡ 4fa15fc6-654d-4a16-9379-2d8caacee1be
newtonDN(g,0.1,2)
...
# ╔═╡ de4d6ac5-51af-414c-a8d2-06181ff916e6
@bind x0 Slider(0.05:0.05:2, default=0.5)

# ╔═╡ 1170ef75-6bd8-404f-a31f-f4dde2f08683
@bind n Slider(1:1:1000, default=500)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "bcb9069d7d07a98073f64a2c8942890122543264"
